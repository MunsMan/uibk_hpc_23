...
int main(int argc, char* argv[]) {
	...
	Particle* particles;
	MPI_Alloc_mem(num_particles * sizeof(Particle), MPI_INFO_NULL, &particles);
	Particle** particles_pointer = malloc(num_particles * sizeof(Particle*));
	Node* nodes = malloc(num_particles * 2 * sizeof(Node));
	PreAlocNodes pre_aloc_nodes = { .nodes = nodes,
		                            .num_nodes = num_particles * 2,
		                            .used_nodes = 0 };
	...
	MPI_Ibcast(&particles[(num_ranks - 1) * step_width],
	           num_particles - (num_ranks - 1) * step_width, MPI_PARTICLE, num_ranks - 1,
	           MPI_COMM_WORLD, &requests[num_ranks - 1]);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Win* windows = malloc(sizeof(MPI_Win) * num_ranks);
	double start;
	start = MPI_Wtime();
	if(my_rank == 0) {
		printf("Number of Rangs: %d with %d Particles and %d Iterations\n", num_ranks,
		       num_particles, numSteps);
	}
	// Run simulation
	for(int step = 0; step < numSteps; step++) {
		barnes_hut(particles_pointer, num_particles, num_ranks, my_rank, &pre_aloc_nodes);
		for(int i = 0; i < num_ranks; i++) {
			MPI_Win_create(particles, sizeof(Particle) * num_particles, sizeof(char), MPI_INFO_NULL,
			               MPI_COMM_WORLD, &windows[i]);
			if(i == my_rank) {
				MPI_Win_fence(0, windows[i]);
			} else {
				MPI_Win_fence(0, windows[i]);
				int size = step_width * sizeof(Particle);
				MPI_Put((void*)&particles[my_rank * step_width], size, MPI_BYTE, i,
				        my_rank * step_width * sizeof(Particle), size, MPI_BYTE, windows[i]);
			}
		}
		for(int i = 0; i < num_ranks; i++) {
			MPI_Win_fence(0, windows[i]);
