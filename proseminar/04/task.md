...
When profiling the program with `perf record` and the analyzing with `perf report` we can observe the following output:
...
We can see that most of the time is spent executing the calculation of temperature of each cell.
Following is a part int `libmlx4-rdmav2.so` which is a library for Infiniband communication suggesting this is some sort of communication with other ranks.

Afterwards with 7% follows the source line 112 which again is part of the loop calculating the cell temperatures.

#### `prof` and `gprof`

**Unfortunately due to an incompatible dwarf version we were not able to execute these profiling methods on the lcc3 cluster. The following profiles were performed on a local machine which limits there usefulness but since performance could only be relative this still gives a good indication of how the program overall behaves and where the hotspots are located.**

![gpref output](./images/1d.png)

If we inspect the output of `gperf` we can see again that a large portion of the time is spent in the functions `writev` and `readv` which indicates most of the time is spent writing and reading from the MPI buffers.
...
Again here we can see that a significant chunk is spent on line 144 which is:
...
#### Changing Cell Computation

The final improvement is the replacement from the cell computation. By just averaging the neighbors, the computation is reduced by one multiplication and 2 additions.
