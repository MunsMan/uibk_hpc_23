...
void printTemperatureDouble(double* X, int n, int m);
...
int main(int argc, char** argv) {
	...
	MPI_Type_contiguous(rankLengthX, MPI_UINT32_T, &topBottom);
	MPI_Type_commit(&topBottom);

	MPI_Type_vector(rankLengthY, 1, bufferLengthX, MPI_UINT32_T, &leftRight);
	...
	for(int j = 0; j < bufferLengthY; j++) {
		for(int i = 0; i < bufferLengthX; i++) {
			A[INDEX(i, j)] = 273 * MULTIPLIER; // temperature is 0° C everywhere (273 K)
			B[INDEX(i, j)] = 273 * MULTIPLIER; // temperature is 0° C everywhere (273 K)
		}
	}
	...
	if(source_x / rankLengthX == x && source_y / rankLengthY == y) {
		...
		A[INDEX(source_x, source_y)] = (273 + 60) * MULTIPLIER;
	} else {
		source_x = -1;
		source_y = -1;
	}
	...
	for(int t = 0; t < T; t++) {
		...
		if(source_y != -1) {
			B[INDEX(source_x, source_y)] = 333 * MULTIPLIER;
		}
	...
	MPI_Type_create_subarray(2, subArraySizes, subArraySubSizes, subArrayStarting, 1, MPI_UINT32_T,
	                         &subarrayType);
	...
	MPI_Type_create_subarray(2, sizes, subsizes, starts, 1, MPI_UINT32_T, &blockType);
	...
	if(myRank == 0) {
		...
		double* finalRes = malloc(N * N * sizeof(double_t));
    bufferLengthX = N;

		for(long long j = 0; j < N; j++) {
			for(long long i = 0; i < N; i++) {
				finalRes[INDEX(i, j)] = ((double)res[INDEX(i, j)]) / (double)MULTIPLIER;
			}
		}
		...
		printf("Final:\t\n");
		printTemperatureDouble(finalRes, N, N);
		printf("\n");

		int success = 1;
		for(long long i = 0; i < N; i++) {
			for(long long j = 0; j < N; j++) {
				value_t temp = finalRes[INDEX(i, j)];
...
void updateOuterCells(Vector A, Vector B, int rankLengthX, int rankLengthY, int bufferLengthX,
	...
	for(long long j = 1; j < rankLengthY + 1; j++) {
		for(long long i = 1; i < rankLengthX + 1; i += rankLengthX - 1) {
			...
			B[INDEX(i, j)] = (tl + tr + tb + ta) >> 2;
		}
	}
}
void printTemperatureDouble(double* X, int bufferLengthX, int bufferLengthY) {
	const char* colors = " .-:=+*^X#%@";
